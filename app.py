...
st.caption("AI Movie Creation Studio")
...
st.markdown("### ⚡ Movie Pipeline")

st.write("1️⃣ Story Creation")
st.write("2️⃣ Scene Breakdown")
st.write("3️⃣ Visual Generation")
st.write("4️⃣ Video Generation")
st.write("5️⃣ Audio Production")
st.write("6️⃣ Movie Assembly")
...
```
st.header("🎥 Create Your Movie")

st.write(
    "Describe your movie idea and VisionCraft AI will create "
    "a complete story structure with characters and scenes."
)
...
story_idea = st.text_area(
    ...
    height=180,
    placeholder=(
        "Example: Create a 20-minute Hindi anime fantasy movie "
        "about six friends who discover powerful Bond Beasts "
        "and must protect their world from an ancient evil."
    )
)
    ...
    if not story_idea.strip():
        ...
        try:
            ...
            with st.status(
                ...
                st.session_state.prepared_scenes = prepared_scenes
                st.session_state.audio_plan = audio_plan

                status.update(
                    label="✅ Movie story created successfully!",
                    state="complete"
                )
...
if st.session_state.story_data:
    ...
    st.header(
        f"🎬 {movie.get('title', 'Untitled Movie')}"
    )
    ...
    with col1:
        ...
        st.metric(
            "Duration",
            f"{movie.get('duration_minutes', 0)} min"
        )
...
if not st.session_state.prepared_scenes:
    ...
    scenes = st.session_state.prepared_scenes

    st.success(
        f"{len(scenes)} scenes are ready for production."
    )
    ...
    with st.expander(
        ...
        st.write(
            f"Music: {scene.get('background_music', '')}"
        )

        st.write(
            f"Sound Effects: "
            f"{scene.get('sound_effects', [])}"
        )
    ...
    if st.button(
        ...
        if not VIDEO_ENGINE_AVAILABLE:
            ...
            st.error(
                ...
                "Check video_engine.py and requirements.txt."
            )
...
if uploaded_image:
    ...
    motion_prompt = st.text_area(
        "Describe the motion",
        placeholder=(
            "The camera slowly moves forward while "
            "the character's clothes move in the wind."
        )
    )
...
```
st.header("🎞️ Scene Generator")

st.info(
    "The complete scene generator is automatically created "
    "when you generate a movie."
)

if st.session_state.prepared_scenes:

    for item in st.session_state.prepared_scenes:
...
st.info(
    ...
    "or Streamlit Secrets. Never place API keys directly in app.py."
)
